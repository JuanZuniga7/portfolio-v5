            ...
            <div className="flex flex-col items-center justify-center mt-16">
                {experienceData && experienceData.map((item, index) => (
                    <HorizontalItem reverse={index % 2 === 0}>
                        <div className="flex flex-col justify-center font-serif h-full w-full text-left px-4">
                            {Object.keys(item).map((key: string, index: number) => {
                                if (key === "position") return <h3 className="text-lg md:text-xl lg:text-2xl xl:text-3xl animate-character font-bold font-mono">{item.position}</h3>
                                else {
                                    return (
